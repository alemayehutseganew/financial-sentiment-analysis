# Stock Market Price Prediction (Task 1)

This repository contains the Week 1 deliverables for the news-driven stock research task. It couples exploratory data analysis, sentiment scoring, and factor correlation notebooks with a reproducible Python environment.
...
├── notebooks/                        # Task 1 exploratory notebooks
...
- Working branch: `task-1` (create via `git checkout -b task-1` if it does not exist).
- Commit cadence: at least **three descriptive commits per day** (e.g., “feat: add topic modeling cell”).
...
- Push frequently; do not commit notebook outputs >5 MB (use `git lfs` or clear outputs if needed).
